...
class DriverStatusController extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    print(
        '🚀 [DriverStatusController] Inicializando DriverStatusController...');
    ...
    try {
      ...
      if (await _safeInitializeDriverIdWithRetry()) {
        ...
        print(
            '❌ [DriverStatusController] Falha ao carregar driver ID durante inicialização');
  ...
  Future<bool?> _loadDriverStatus(String driverId) async {
    try {
      final effectiveStatus =
          await _driverService.getDriverEffectiveStatus(driverId);
  ...
  Future<void> toggleOnlineStatus() async {
    ...
    print(
        '💾 [DRIVER_STATUS_CONTROLLER] Estado anterior salvo: $currentStatus');
    ...
    print(
        '🔀 [DRIVER_STATUS_CONTROLLER] Decidindo próxima ação baseada no status anterior: $currentStatus');
    try {
      if (currentStatus == DriverOnlineStatus.online) {
        print(
            '📴 [DRIVER_STATUS_CONTROLLER] Estado anterior era ONLINE, indo para OFFLINE...');
        await _goOffline();
        print('✅ [DRIVER_STATUS_CONTROLLER] _goOffline concluído com sucesso');
      } else {
        print(
            '📱 [DRIVER_STATUS_CONTROLLER] Estado anterior era OFFLINE, verificando elegibilidade...');

        // Verificar se pode ficar online (documentos aprovados, etc.)
        final eligibilityStatus =
            await _driverService.getOnlineEligibilityStatus(_driverId!);
        ...
        if (eligibilityStatus['canGoOnline'] == true) {
          ...
          // Notificar erro via callback
          _notifyEligibilityError(eligibilityStatus);

          // Não lançar exceção - o callback já cuida de mostrar o erro à UI
          return;
        }
      }
      ...
      print(
          '❌ [DRIVER_STATUS_CONTROLLER] Erro em toggleOnlineStatus: ${e.toString()}');
      ...
      print(
          '🔄 [DRIVER_STATUS_CONTROLLER] Garantindo que status volte para OFFLINE devido ao erro...');
      _updateStatus(_status.copyWith(
        status: DriverOnlineStatus.offline,
        lastStatusChange: DateTime.now(),
      ));
  ...
  Future<void> _goOnline() async {
    ...
    if (_driverId == null) {
      print(
          '❌ [DRIVER_STATUS_CONTROLLER] Driver ID é nulo, voltando para offline');
      print(
          '🔄 [DRIVER_STATUS_CONTROLLER] Atualizando status para OFFLINE devido a driver ID nulo');
      _updateStatus(_status.copyWith(
        status: DriverOnlineStatus.offline,
        lastStatusChange: DateTime.now(),
      ));
    ...
    if (_driverId!.isEmpty) {
      print(
          '❌ [DRIVER_STATUS_CONTROLLER] Driver ID inválido (vazio), voltando para offline');
      print(
          '🔄 [DRIVER_STATUS_CONTROLLER] Atualizando status para OFFLINE devido a driver ID vazio');
      _updateStatus(_status.copyWith(
        status: DriverOnlineStatus.offline,
        lastStatusChange: DateTime.now(),
      ));
    ...
    try {
      print(
          '🔗 [DRIVER_STATUS_CONTROLLER] Atualizando driver no banco (isOnline: true)...');
      print(
          '   🔹 Chamando _driverService.updateAvailability($_driverId!, true)');
      // Atualizar a intenção online no banco de dados
      await _driverService.updateAvailability(_driverId!, true);
      print(
          '✅ [DRIVER_STATUS_CONTROLLER] Driver atualizado no banco com sucesso');

      _onlineStartTime = DateTime.now();
      print(
          '⏰ [DRIVER_STATUS_CONTROLLER] Online start time definido: $_onlineStartTime');

      print(
          '🔄 [DRIVER_STATUS_CONTROLLER] Atualizando status local para ONLINE...');
      _updateStatus(_status.copyWith(
        status: DriverOnlineStatus.online,
        lastStatusChange: DateTime.now(),
      ));
      print(
          '✅ [DRIVER_STATUS_CONTROLLER] Status atualizado para ONLINE localmente');
      ...
      print(
          '🔄 [DRIVER_STATUS_CONTROLLER] Revertendo status para OFFLINE devido ao erro');
      _updateStatus(_status.copyWith(
        status: DriverOnlineStatus.offline,
        lastStatusChange: DateTime.now(),
      ));
      print('✅ [DRIVER_STATUS_CONTROLLER] Status revertido para OFFLINE');
      print(
          '⚠️ [DRIVER_STATUS_CONTROLLER] Relançando exceção para a UI tratar');
  ...
  Future<void> _goOffline() async {
    ...
    if (_driverId != null) {
      ...
      if (_driverId!.isEmpty) {
        print(
            '❌ [DRIVER_STATUS_CONTROLLER] Driver ID inválido (vazio), pulando atualização no banco');
      } else {
        try {
          print(
              '🔗 [DRIVER_STATUS_CONTROLLER] Atualizando driver no banco (isOnline: false)...');
          print(
              '   🔹 Chamando _driverService.updateAvailability($_driverId!, false)');
          ...
          print(
              '⚠️ [DRIVER_STATUS_CONTROLLER] Erro ao atualizar no banco (continuando): $e');
      ...
      print(
          '⚠️ [DRIVER_STATUS_CONTROLLER] Driver ID é nulo, pulando atualização no banco');
    }
    ...
    print(
        '🔄 [DRIVER_STATUS_CONTROLLER] Atualizando status local para OFFLINE...');
    _updateStatus(_status.copyWith(
      status: DriverOnlineStatus.offline,
      lastStatusChange: DateTime.now(),
    ));
  ...
  void _startOnlineTimer() {
    _onlineTimer = Timer.periodic(const Duration(minutes: 1), (_) {
      if (_onlineStartTime != null) {
        final newOnlineTime =
            DateTime.now().difference(_onlineStartTime!) + _status.onlineTime;
  ...
  void _notifyEligibilityError(Map<String, dynamic> eligibilityStatus) {
    ...
    if (onEligibilityError != null) {
      ...
      print(
          '⚠️ [DRIVER_STATUS_CONTROLLER] Nenhum callback de erro configurado');
    }
  }

  /// Tenta carregar o driverId com retry e backoff exponencial
  Future<bool> _loadDriverIdWithRetry(
      {int maxRetries = 3,
    ...
    while (attempt < maxRetries) {
      try {
        print(
            '🔄 [DRIVER_STATUS_CONTROLLER] Tentativa ${attempt + 1}/$maxRetries para carregar driver ID');

        print('🔗 [DRIVER_STATUS_CONTROLLER] Buscando usuário atual...');
        final user = await UserService.getCurrentUser();
        print(
            '📊 [DRIVER_STATUS_CONTROLLER] Usuário atual obtido: ${user?.id}');

        if (user != null) {
          print(
              '🔗 [DRIVER_STATUS_CONTROLLER] Buscando driver ID para usuário ${user.id}...');
          _driverId = await WalletService().getDriverIdForUser(user.id);
          print('📊 [DRIVER_STATUS_CONTROLLER] Driver ID obtido: $_driverId');

          if (_driverId != null) {
            print(
                '✅ [DRIVER_STATUS_CONTROLLER] Driver ID carregado com sucesso: $_driverId');
            return true;
          } else {
            print(
                '⚠️ [DRIVER_STATUS_CONTROLLER] Driver ID é null para usuário ${user.id}');
          }
        } else {
        ...
        print(
            '⚠️ [DRIVER_STATUS_CONTROLLER] Tentativa ${attempt + 1} falhou, aguardando ${delay.inSeconds}s...');
        await Future.delayed(delay);
        delay *= 2; // Backoff exponencial
        attempt++;
      } catch (e, stackTrace) {
        print(
            '❌ [DRIVER_STATUS_CONTROLLER] Erro na tentativa ${attempt + 1}: $e');
        print('❌ [DRIVER_STATUS_CONTROLLER] Stack trace: $stackTrace');
    ...
    print(
        '⚠️ [DRIVER_STATUS_CONTROLLER] Todas as tentativas esgotadas sem sucesso');
    return false;
  }
  ...
  Future<bool> _safeInitializeDriverIdWithRetry() async {
    print(
        '🔵 [DRIVER_STATUS_CONTROLLER] _safeInitializeDriverIdWithRetry iniciado');
    ...
    if (_isInitialized && _driverId != null) {
      print(
          '✅ [DRIVER_STATUS_CONTROLLER] Driver ID já está inicializado: $_driverId');
      return true;
    }

    print(
        '🔗 [DRIVER_STATUS_CONTROLLER] Driver ID não inicializado, chamando _loadDriverIdWithRetry...');
    final result = await _loadDriverIdWithRetry();
    print(
        '📊 [DRIVER_STATUS_CONTROLLER] Resultado de _loadDriverIdWithRetry: $result');
    ...
    if (result && _driverId != null) {
      ...
      print(
          '⚠️ [DRIVER_STATUS_CONTROLLER] _loadDriverIdWithRetry retornou true mas _driverId ainda é null');
    }
