...
import '../../data/models/supabase/trip_request.dart';
import '../../data/models/trip_request_data.dart';
...
class TripRequestManager {
  ...
  Future<void> handleDriverResponse({
    ...
    try {
      ...
      // Buscar dados do motorista e da solicitação
      final results = await Future.wait([
        _supabase
            .from('drivers')
            .select('app_users(full_name, phone), vehicle_brand, vehicle_model, vehicle_plate')
            .eq('id', driverId)
            .single(),
        _supabase
            .from('trip_requests')
      ...
      final driverName = driverData['app_users']?['full_name'] as String? ?? 'Motorista';
      final driverPhone = driverData['app_users']?['phone'] as String?;
      final vehicleBrand = driverData['vehicle_brand'] as String?;
      final vehicleModel = driverData['vehicle_model'] as String?;
      final vehiclePlate = driverData['vehicle_plate'] as String?;
      final fullVehicleModel = vehicleBrand != null && vehicleModel != null 
      ...
      if (accepted) {
        ...
        if (FeatureFlags().enablePushNotifications) {
          await _notificationService.sendPassengerNotification(
            passengerId: passengerId,
  ...
  Future<void> _notifyPassengerNoDriverFound(String passengerId) async {
    try {
      await _notificationService.sendPassengerNotification(
        passengerId: passengerId,
        tripRequestId: '', // No specific request ID for this notification
        notificationType: 'trip_expired',
        driverName: '', // No driver for this notification
      );
    } catch (e) {
