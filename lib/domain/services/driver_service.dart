...
import '../../data/models/supabase/driver_offer.dart';
import '../../data/models/supabase/driver_effective_status.dart';
import '../../data/models/supabase/trip.dart';
import '../../data/models/vehicle_category.dart';
import '../../core/validators/database_constraints_validator.dart';
...
class DriverService {
  ...
  Future<Driver?> getDriverByUserId(String userId) async {
    ...
    AppLogger.process('Iniciando busca de motorista por userId', tag: 'DRIVER_SERVICE');
    AppLogger.query('drivers', 1, tag: 'DRIVER_SERVICE', filters: {'user_id': userId});

    try {
      AppLogger.process('Executando query no Supabase', tag: 'DRIVER_SERVICE');
      final response =
          await _supabase.from('drivers').select().eq('user_id', userId).single();

      final driver = Driver.fromJson(response);
      final duration = DateTime.now().difference(startTime);
      
      AppLogger.performance('get_driver_by_user_id', duration, tag: 'DRIVER_SERVICE');
      AppLogger.success('Motorista encontrado por userId', tag: 'DRIVER_SERVICE');
      AppLogger.read('Driver', driver.id, tag: 'DRIVER_SERVICE');
      
      return driver;
    } on PostgrestException catch (e) {
      AppLogger.error('PostgrestException ao buscar motorista', tag: 'DRIVER_SERVICE', error: e);
      AppLogger.debug('PostgrestException detalhes', tag: 'DRIVER_SERVICE', data: {
        'code': e.code,
        'message': e.message,
        'details': e.details,
        'hint': e.hint
      ...
      if (e.code == 'PGRST116') {
        AppLogger.warning('Motorista não encontrado por userId', tag: 'DRIVER_SERVICE');
        return null;
      }
  ...
  Future<Map<String, dynamic>?> getDriverWithUserData(String driverId) async {
    try {
      ...
      try {
        ...
        print(
            '⚠️ View otimizada não disponível para getDriverWithUserData, usando fallback: $e');
      }
  ...
  Future<Driver> createDriver({
    ...
    try {
      ...
      print(
          '🔍 [DriverService.createDriver] Placa vazia: ${plate.trim().isEmpty}');
      print(
          '🔍 [DriverService.createDriver] Começa com PENDENTE: ${plate.trim().startsWith('PENDENTE')}');

      // Verificar se a placa já existe antes da criação
      if (plate.trim().isNotEmpty && !plate.trim().startsWith('PENDENTE')) {
        print(
            '🔍 [DriverService.createDriver] Placa válida detectada, verificando unicidade...');
        try {
          await _checkVehiclePlateUniquenessForCreation(plate.trim());
          print(
              '✅ [DriverService.createDriver] Verificação de placa concluída com sucesso');
        } catch (e) {
          print(
              '❌ [DriverService.createDriver] Erro durante verificação de placa: $e');
          // Re-throw para manter o comportamento original
          rethrow;
        }
      } else {
        print(
            '🔍 [DriverService.createDriver] Placa ignorada (vazia ou começa com PENDENTE)');
      }
      ...
      if (categoryStr.isNotEmpty) {
        final isValidCategory =
            _vehicleCategoryValidator.isVehicleCategoryValid(categoryStr);
        if (!isValidCategory) {
          print(
              '❌ [DriverService.createDriver] vehicle_category inválido: $categoryStr');
          throw ValidationException(
              'Categoria de veículo inválida: $categoryStr');
        }
        print(
            '✅ [DriverService.createDriver] vehicle_category válido: $categoryStr');
      }
      ...
      print(
          '❌ [DriverService.createDriver] PostgrestException: ${e.code} - ${e.message}');
  ...
  static Future<void> updateDriver(
    ...
    if (categoryStr.isNotEmpty) {
      ...
      try {
        await validator.validateVehicleCategoryExists(categoryStr);
        print(
            '✅ [DriverService.updateDriver] vehicle_category válido: $categoryStr');
      } catch (e) {
        print(
            '❌ [DriverService.updateDriver] vehicle_category inválido: $categoryStr - $e');
        throw ValidationException(
            'Categoria de veículo inválida: $categoryStr');
      }
    }
  ...
  Future<List<DriverOffer>> getPendingOffers(String driverId) async {
    try {
      ...
      throw const DatabaseException(
          'Erro inesperado ao buscar ofertas pendentes');
    }
  }
  ...
  Future<void> updateAvailability(String driverId, bool isOnline) async {
    ...
    try {
      ...
      print(
          '❌ [DRIVER_SERVICE] PostgrestException em updateAvailability: ${e.code} - ${e.message}');
      print('❌ [DRIVER_SERVICE] Details: ${e.details}');
      print('❌ [DRIVER_SERVICE] Hint: ${e.hint}');
      throw PostgrestErrorMapper.mapError(e);
    } catch (e, stackTrace) {
      print(
          '❌ [DRIVER_SERVICE] Erro inesperado em updateAvailability: ${e.toString()}');
  ...
  Stream<List<Trip>> streamDriverActiveTrips(String driverId) => _supabase
      ...
      .map(
        (data) => data
            .where(
                (trip) => ['accepted', 'in_progress'].contains(trip['status']))
            .map(Trip.fromJson)
            .toList(),
      );
  ...
  Future<List<Driver>> getAvailableDriversNearby({
    ...
    try {
      ...
      final lngDelta = radiusKm /
          (111.0 * math.cos(latitude * math.pi / 180.0))
              .abs()
      ...
      print(
          '  - lng range: ${longitude - lngDelta} to ${longitude + lngDelta}');
      ...
      try {
        ...
        try {
          ...
          final isMissingAverageRating = e2.code == '42703' ||
              msg.contains('average_rating') ||
              msg.contains('column');
          if (isMissingAverageRating) {
            if (kDebugMode) {
              debugPrint(
                  'getAvailableDriversNearby: coluna average_rating ausente. Aplicando fallback sem ordenação. (${e2.code})');
            }
        ...
        for (int i = 0; i < (response).length; i++) {
          ...
          final mappedData = {
            ...
            'vehicle_plate': driverData['vehicle_plate'] ??
                '', // Assumindo que pode não existir na view

            'approval_status':
                'approved', // Só motoristas aprovados aparecem na view
      ...
      throw const DatabaseException(
          'Erro inesperado ao buscar motoristas disponíveis.');
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getAvailableDriversNearbyWithUserData({
    ...
    print(
        '🔍 [${DateTime.now()}] Iniciando getAvailableDriversNearbyWithUserData...');
    ...
    try {
      ...
      print(
          '🎯 [${DateTime.now()}] Busca finalizada com ${limitedDriversData.length} motoristas com dados de usuário');

      return limitedDriversData;
    } on PostgrestException catch (e) {
      print(
          '❌ [${DateTime.now()}] PostgrestException em getAvailableDriversNearbyWithUserData:');
      print('  - Código: ${e.code}');
      print('  - Mensagem: ${e.message}');
      throw PostgrestErrorMapper.mapError(e);
    } catch (e) {
      print(
          '❌ [${DateTime.now()}] Erro inesperado em getAvailableDriversNearbyWithUserData:');
      print('  - Tipo: ${e.runtimeType}');
      print('  - Mensagem: $e');
      throw const DatabaseException(
          'Erro inesperado ao buscar motoristas disponíveis.');
  ...
  Future<List<VehicleCategoryData>>
      _createCategoryDataWithoutDriverCount() async {
    final categoryDataList = <VehicleCategoryData>[];
  ...
  Future<VehicleCategoryData?> getCategoryData(
    VehicleCategory category, {
    required double latitude,
    required double longitude,
  }) async {
  ...
  Future<DriverEffectiveStatus?> getDriverEffectiveStatus(
  ...
  Future<Map<String, dynamic>> getOnlineEligibilityStatus(
      String driverId) async {
    print(
        '🔍 [DRIVER_SERVICE] getOnlineEligibilityStatus iniciado para driver: $driverId');
    ...
    try {
      ...
      if (approvalStatus != 'approved') {
        ...
        switch (approvalStatus) {
          ...
          case 'rejected':
            message =
                'Seu perfil foi rejeitado. Entre em contato com o suporte.';
      ...
      final documentsStatus =
          await _driverStatusService.checkRequiredDocumentsApproved(driverId);
      ...
      if (!documentsStatus['allApproved']) {
        ...
        String message =
            'Você precisa ter todos os documentos aprovados para ficar online.';
        String actionRequired =
            'Complete o envio e aprovação dos documentos obrigatórios.';

        if (missingDocs.isNotEmpty) {
          message =
              'Documentos obrigatórios não enviados: ${missingDocs.join(', ')}.';
          ...
          message =
              'Documentos aguardando aprovação: ${pendingDocs.join(', ')}.';
      ...
      final canGoOnlineNow =
          await _driverStatusService.canDriverGoOnlineNow(driverId);
      ...
      if (!canGoOnlineNow) {
        return {
          ...
          'message':
              'Você só pode ficar online durante seus horários de trabalho configurados.',
          'actionRequired':
              'Configure seus horários ou aguarde o horário permitido.',
        };
      }
  ...
  Future<bool> canDriverGoOnline(String driverId) async {
    print(
        '🔵 [DRIVER_SERVICE] canDriverGoOnline iniciado para driver: $driverId');
    ...
    try {
      print(
          '🔗 [DRIVER_SERVICE] Chamando _driverStatusService.canDriverGoOnlineNow...');
      final canGoOnline =
          await _driverStatusService.canDriverGoOnlineNow(driverId);
      print(
          '📊 [DRIVER_SERVICE] Resultado: $canGoOnline para driver: $driverId');
  ...
  Future<List<DriverEffectiveStatus>> getEffectivelyOnlineDrivers() async =>
      await _driverStatusService.getOnlineDrivers();

  /// Obtém estatísticas de status dos motoristas
  Future<Map<String, int>> getDriverStatusStats() async =>
      await _driverStatusService.getDriverStatusStats();

  /// Verifica se a placa do veículo já está sendo usada por outro motorista
  Future<void> _checkVehiclePlateUniqueness(
      String plate, String currentDriverId) async {
    print(
        '🔍 [DriverService._checkVehiclePlateUniqueness] Verificando unicidade da placa: "$plate" para motorista: $currentDriverId');
    try {
      final response = await _supabase
          .from('drivers')
      ...
      print(
          '🔍 [DriverService._checkVehiclePlateUniqueness] Resultado da consulta: ${response.length} registros encontrados');
      if (response.isNotEmpty) {
        print(
            '❌ [DriverService._checkVehiclePlateUniqueness] Placa já existe: ${response.map((r) => r['id']).toList()}');
        throw const ValidationException(
          'Esta placa já está cadastrada por outro motorista. Por favor, verifique os dados e tente novamente.',
        );
      }
      print(
          '✅ [DriverService._checkVehiclePlateUniqueness] Placa disponível: "$plate"');
    } on PostgrestException catch (e) {
      print(
          '❌ [DriverService._checkVehiclePlateUniqueness] PostgrestException: ${e.code} - ${e.message}');
      throw PostgrestErrorMapper.mapError(e);
    } catch (e) {
      print(
          '❌ [DriverService._checkVehiclePlateUniqueness] Erro inesperado: $e');
  ...
  Future<void> _checkVehiclePlateUniquenessForCreation(String plate) async {
    try {
      print(
          '🔍 [_checkVehiclePlateUniquenessForCreation] Verificando unicidade da placa: "$plate"');
      ...
      print(
          '🔍 [_checkVehiclePlateUniquenessForCreation] Resposta do Supabase: $response');

      if (response.isNotEmpty) {
        print(
            '❌ [_checkVehiclePlateUniquenessForCreation] Placa já existe no banco!');
        throw const ValidationException(
          'Esta placa já está cadastrada por outro motorista. Por favor, verifique os dados e tente novamente.',
        );
      }
      ...
      print(
          '❌ [_checkVehiclePlateUniquenessForCreation] PostgrestException: ${e.code} - ${e.message}');
      throw PostgrestErrorMapper.mapError(e);
    } catch (e) {
  ...
  Future<void> _validateUpdateData(Map<String, dynamic> updates) async {
    // DEBUG: Log de entrada para validação
    print(
        '🔍 [DriverService._validateUpdateData] Iniciando validação. Campos: ${updates.keys.toList()}');
    ...
    if (updates.containsKey('vehicle_category')) {
      ...
      print(
          '🔍 [DriverService._validateUpdateData] Validando vehicle_category: $category');
      if (category == null) {
      ...
      if (categoryStr.isNotEmpty) {
        try {
          await _vehicleCategoryValidator.validateVehicleCategoryExists(categoryStr);
          print(
              '✅ [DriverService._validateUpdateData] vehicle_category válido: $categoryStr');
        } catch (e) {
          print(
              '❌ [DriverService._validateUpdateData] vehicle_category inválido: $categoryStr - $e');
          throw ValidationException(
              'Categoria de veículo inválida: $categoryStr');
        }
      }
    ...
    if (updates.containsKey('vehicle_plate')) {
      ...
      print(
          '🔍 [DriverService._validateUpdateData] Validando vehicle_plate: $plate');
      if (plate != null) {
        final plateStr = plate.toString().trim();
        print(
            '🔍 [DriverService._validateUpdateData] Placa após trim: "$plateStr"');

        if (plateStr.isNotEmpty && !plateStr.startsWith('PENDENTE')) {
          final cleanPlate =
              plateStr.replaceAll(RegExp(r'[^A-Z0-9]'), '').toUpperCase();
          print(
              '🔍 [DriverService._validateUpdateData] Placa limpa: "$cleanPlate"');

          if (cleanPlate.length != 7) {
            print(
                '❌ [DriverService._validateUpdateData] Placa com tamanho inválido: ${cleanPlate.length} caracteres');
            throw ValidationException(
                'Placa deve ter exatamente 7 caracteres (ex: ABC1234). Tamanho atual: ${cleanPlate.length}');
          }
          ...
          print(
              '🔍 [DriverService._validateUpdateData] Testando placa: "$cleanPlate"');
          print(
              '🔍 [DriverService._validateUpdateData] Regex antigo: ${oldPlateRegex.pattern} - Match: ${oldPlateRegex.hasMatch(cleanPlate)}');
          print(
              '🔍 [DriverService._validateUpdateData] Regex novo: ${newPlateRegex.pattern} - Match: ${newPlateRegex.hasMatch(cleanPlate)}');

          if (!newPlateRegex.hasMatch(cleanPlate)) {
            print(
                '❌ [DriverService._validateUpdateData] Placa não corresponde ao novo regex');
            print(
                '❌ [DriverService._validateUpdateData] Formatos válidos: ABC1234 (antigo) ou ABC1D23 (Mercosul)');
            throw ValidationException(
                'Formato de placa inválido. Use ABC1234 (antigo) ou ABC1D23 (Mercosul). Placa recebida: $plateStr');
          }
          print(
              '✅ [DriverService._validateUpdateData] Placa válida: "$cleanPlate"');
        } else {
          print(
              '⚠️ [DriverService._validateUpdateData] Placa vazia ou começa com PENDENTE: "$plateStr"');
        }
      } else {
        print(
            '⚠️ [DriverService._validateUpdateData] Placa é nula, pulando validação');
      }
    }
      ...
      if (year != null) {
        ...
        if (yearInt != null) {
          ...
          if (yearInt < 1990 || yearInt > currentYear + 1) {
            throw ValidationException(
                'vehicle_year deve estar entre 1990 e ${currentYear + 1}: $yearInt');
          }
        }
      }
    }
    ...
    for (final field in requiredFields) {
      if (updates.containsKey(field)) {
        ...
        if (value == null || value.toString().trim().isEmpty) {
          throw ValidationException(
              '$field é obrigatório e não pode estar vazio');
        }
      }
    }
  }
