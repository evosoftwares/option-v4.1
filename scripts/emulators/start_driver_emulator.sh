...
# Script para iniciar emulador Android para MOTORISTA - OPTION App
...
echo -e "${BLUE}🚗 Iniciando Emulador MOTORISTA - OPTION App${NC}"
echo -e "${BLUE}================================================${NC}"

# Configurações do emulador
AVD_NAME="OPTION_Driver_Pixel8"
...
if ! "$EMULATOR_PATH" -list-avds | grep -q "$AVD_NAME"; then
    ...
    echo -e "${YELLOW}💡 Criando AVD para motorista...${NC}"
...
echo -e "${GREEN}🚀 Iniciando emulador para MOTORISTA...${NC}"
echo -e "${BLUE}Configurações:${NC}"
echo -e "  • AVD: $AVD_NAME"
echo -e "  • DNS: 8.8.8.8, 8.8.4.4"
echo -e "  • Porta: 5554 (padrão para motorista)"
echo -e "  • Snapshot: desabilitado para teste limpo"
echo -e "  • GPU: auto"
echo -e "  • RAM: 4GB"

# Iniciar emulador com configurações específicas para motorista
"$EMULATOR_PATH" \
    -avd "$AVD_NAME" \
...
if adb -s emulator-5554 shell echo "test" >/dev/null 2>&1; then
    echo -e "${GREEN}✅ Emulador MOTORISTA iniciado com sucesso!${NC}"
    echo -e "${GREEN}📱 Device ID: emulator-5554${NC}"
    ...
    # Configurar localização para São Paulo (para testes de GPS)
    ...
    echo -e "${GREEN}🎯 Para executar o app MOTORISTA:${NC}"
    echo -e "${YELLOW}flutter run -d emulator-5554${NC}"
    echo -e "${GREEN}🔗 Para conectar especificamente:${NC}"
    echo -e "${YELLOW}adb connect emulator-5554${NC}"
